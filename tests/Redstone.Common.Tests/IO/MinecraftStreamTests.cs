...
{
    /// <summary>
    /// Unit tests for the <see cref="MinecraftStream"/>.
    /// </summary>
    /// <remarks>
    /// All tests cases are from https://wiki.vg/Protocol#VarInt_and_VarLong
    /// Thank you to the authors.
    /// </remarks>
    public class MinecraftStreamTests
    {
        ...
        public void MinecraftPacketReadVarInt32Test(int expectedValue, byte[] content)
        {
            using var stream = new MinecraftStream(content);

            int value = stream.ReadVarInt32();
        ...
        public void MinecraftPacketReadVarInt64Test(long expectedValue, byte[] content)
        {
            using var stream = new MinecraftStream(content);

            long value = stream.ReadVarInt64();
        ...
        public void MinecraftPacketReadUUIDTest(Guid expectedValue, byte[] content)
        {
            using var stream = new MinecraftStream(content);

            Guid value = stream.ReadUUID();
        ...
        public void MinecraftPacketWriteVarInt32Test(int valueToWrite, byte[] expectedContent)
        {
            ...
            stream.WriteVarInt32(valueToWrite);

            Assert.Equal(expectedContent, stream.Buffer.Take(expectedContent.Length));
        }
        ...
        public void MinecraftPacketWriteVarInt64Test(long valueToWrite, byte[] expectedContent)
        {
            ...
            stream.WriteVarInt64(valueToWrite);
